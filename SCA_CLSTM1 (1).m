clc;
clear;
rng(0);         %change rng value for different channel realizations
load("ThetaOptimizerTrainedModel.mat");

% Parameters
max_iter = 20;         % Maximum number of SCA iterations (Change to 10000 this for monte carlo)
K = 4;
M = 4;
N = 16;
error_prob = 10^-6;
...
for iter = 1:max_iter
    ...
    p_opt = p;
      ...
      cvx_begin 
        ...
        % Approximated L_plus and L_minus
        for k = 1:K
            L_plus_app(k) = mk_new(k) .* ((log(interf_power(k) + p_opt(k)*(abs(initial_channel_new(k,:)*W_fixed(:,k)).^2)+1e-8))/log(2)) + log(mk_new(k)+1e-8)./log(2);
        end 
...
figure;
plot(1:iter, sumrate_history(1:iter), 'o-', 'LineWidth', 2);
xlabel('Number of Iterations');
ylabel('Optimized Sumrate');
title('Optimized Sumrate vs Number of Iterations');
grid on;
